...
test_does_not_include_ignored_directory() {
    rm -rf work && mkdir work && cd work
    create_workspace_with_default_repos WORKSPACE

    mkdir ignored
    echo "int* add() {" >> ignored/bad.cpp
    echo "    return 1 + 1;" >> ignored/bad.cpp
    echo "}" >> ignored/bad.cpp
    ...
    touch WORKSPACE
    echo "cc_binary(" >> BUILD
    echo "  name = \"main\"," >> BUILD
    echo "  srcs = glob([\"**/*.cpp\"])," >> BUILD
    echo ")" >> BUILD

    bazel build :main || "Expected success"
}
